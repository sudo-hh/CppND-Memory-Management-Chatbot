...
void ChatLogic::LoadAnswerGraphFromFile(std::string filename)
{
    ...
    // add pointer to chatlogic so that _chatbot answers can be passed on to the GUI
    _chatBot->SetChatLogicHandle(this);

    // add _chatbot to graph root node
    _chatBot->SetRootNode(rootNode);
    rootNode->MoveChatbotHere(std::move(*_chatBot));

    std::cout << "Exiting scope of function LoadAnswerGraphFromFile()..." << std::endl;
